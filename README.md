# 👋 Hi, I'm Micah Traya

## IT Professional | Automation & AI Workflow Specialist

I am an IT professional with **8+ years of experience** in technical support, cloud administration, enterprise systems, and global service desk operations.

Alongside my IT background, I am developing hands-on expertise in **workflow automation, AI tools, and business process automation** using n8n and Zapier.

### 🚀 What I Work With

- ⚙️ **Automation:** n8n | Zapier | Webhooks | Workflow Automation
- 🤖 **AI:** ChatGPT | Microsoft Copilot | Claude
- 💼 **Business Automation:** Lead Qualification | Customer Support | Email Automation | Inventory Automation
- ☁️ **IT & Cloud:** Microsoft 365 | Active Directory | Azure | AWS | Google Workspace | ServiceNow
- 📋 **Productivity:** Notion | Trello | Technical Documentation | Process Improvement

## 📂 Featured Portfolio

### n8n Automation Portfolio

A collection of 10 practical automation projects demonstrating workflow automation, AI integration, business process automation, and client-focused solutions.

01 — Age Verification
02 — Social Media Advertisement Generator
03 — Customer Support Ticket Router
04 — Leave Request Approval
05 — TikTok Affiliate Content Generator
06 — Smart Inventory Alert System
07 — AI Social Media Caption Generator
08 — AI Customer Support Assistant
09 — AI Lead Qualification System
10 — Client-Style Business Automation

Each project demonstrates how workflow automation can be used to reduce repetitive tasks, improve business processes, and support day-to-day operations.

## 🎯 Career Focus

I am interested in opportunities involving:

- Tech VA Support
- IT Support
- Workflow Automation
- AI Automation
- Business Process Automation
- Customer Support Automation
- Lead Management
- CRM Automation

## 🔗 Connect With Me

**LinkedIn:** [Micah Traya](https://www.linkedin.com/in/micah-traya/)

**Email:** micahtraya16@gmail.com

**Contact no.:** +63 9454563770 

---

⭐ **View my [n8n Automation Portfolio](https://github.com/micahtraya/n8n-automation-portfolio)**
